...
camera_calibration_parameters_filename = 'calibration_chessboardDEXP720.yaml' #калибровочный файл для камеры


# Start the video stream
cap = cv2.VideoCapture(1) #аргумент - id камеры
...
while(True):
    ...
    ret, frame = cap.read()
    ret, frame = cap.read()
    frame,x,y,z,a_x,a_y,a_z = odom.updateCameraPoses(frame,time.time()*1000-startTime,8) #следим за маркером с id 8
    cv2.imshow("im",frame)
    cv2.waitKey(1)